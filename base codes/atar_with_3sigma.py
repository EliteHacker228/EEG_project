...
for i in range(len(event_times) - 1):
    ...
    for w in range(n_windows):
        ...
        if sum(artifact_flags) >= min_channels_with_artifact:
            print(f"[Сегмент {i+1}/{len(event_times)-1}, окно {w+1}/{n_windows}] Глобальный артефакт обнаружен. Применяется ATAR.")
            for ch in range(data.shape[0]):
            ...
            print(f"[Сегмент {i+1}/{len(event_times)-1}, окно {w+1}/{n_windows}] Глобальный артефакт не обнаружен. Пропуск.")
